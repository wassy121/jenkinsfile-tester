...
Paste a `Jenkinsfile`, get back structured JSON: parse errors with line and column, named validation diagnostics (errors, warnings, suggestions), and a suite of structural assertions. Runs entirely in the browser or in Node.js via WebAssembly.
...
environment {
  ...
  DB_PASSWORD = credentials('db-secret') // EnvValue::Credentials — W002/W005 are suppressed (correct usage)
}
```
...
| Limitation | Notes |
|---|---|
| `script {}` bodies | Captured as opaque raw text — arbitrary Groovy is not re-parsed |
